...
pca_centralities(centralities_bar)


pca_centralities(centralities_nobar)
```
...
bar_attendees <- 
  gonnet_df %>% 
  filter(attended_bar == TRUE) %>% 
  select(nodes) %>% unlist() %>% as.vector()

# the matrix with only bar attendees, and no bar
gonnet[,bar_attendees][-1,] %>% sum()
...
# nodefactor
ergm_model5 <- ergm(gonnet_net_nobar ~ edges + nodefactor("bar"))
# heterophily on bar attendance
ergm_model6 <- ergm(gonnet_net_nobar ~ edges + nodemix("bar"))
```
