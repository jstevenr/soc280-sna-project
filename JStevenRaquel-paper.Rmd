...
# boxplot comparing degree
degree_gender <- gonnet_df2 %>% 
  select(nodes, gender, degree) %>% 
  drop_na()

# boxplot of degree by gender
ggplot(degree_gender, aes(x=gender, y=degree)) + 
  geom_boxplot() +
  ggtitle("Boxplot of Degree, Males vs Females") +
  theme_bw()
```
...
gonnet_df2 %>% 
  filter(!is.na(gender)) %>%
  group_by(gender) %>%
  summarize(average_degree = mean(degree)) %>% 
...
# H0: the variance in degree between the two genders is equal
degree_gender$gender <- as.factor(degree_gender$gender)
  
leveneTest(degree ~ gender, 
           data = degree_gender)
# p-value > 0.05, so we fail to reject H0, conclude the variances are equal
```

```{r student-t-test-for-degree}
# Student's t-test comparing the mean degree of men and women
# H0: mu_x - mu_y = 0
t.test(x = degree_m, 
       y = degree_f,
...
```{r bar-attendance-degree}
# boxplot comparing distribution of degree between bar (not)-attended
ggplot(gonnet_df2 %>% filter(!(nodes %in% c("b", "x", "x2"))), 
       aes(x = attended_bar, degree)) + 
  geom_boxplot() + theme_bw()

# average degree among bar attendees
degree_bar <- gonnet_df2 %>% 
  select(nodes, attended_bar, degree) %>%
  filter(!(nodes %in% c("b", "x2", "x"))) %>%
  filter(attended_bar == TRUE)

# average degree among non-bar attendees
...
# H0: mu_x - mu_y = 0
t.test(x = degree_bar$degree,
       y = degree_nobar$degree,
       var.equal = FALSE, alternative = "two.sided")
# p-value < 0.05, reject H0
# conclude the difference in mean degree is not equal to zero
```


```{r gplot-degree, echo = F}
...
legend("topright",
       ...
       fill = F, border = "white", pch = c(0, 2),
       title = "Gender", bty = "n")
title("Gonorrhea network, sized by degree")
```
...
```{r ergm-data}
gonnet.degreedist <- table(sna::degree(gonnet, cmode = "degree"))
gonnet.degreedist
...
# testing bar attendance effect on degree
